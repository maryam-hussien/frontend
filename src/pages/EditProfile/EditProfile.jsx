...
const EditProfile = () => {
  ...
  const getUsers = async () => {
    try {
      const response = await axios.get("https://backend-swart-theta.vercel.app/user/find", {
        headers: {
  ...
  const handleSave = () => {
      try {
        const response = axios.put('https://backend-swart-theta.vercel.app/user/edit', userData, {
          headers: {
            "Content-Type": "application/json",
            authorization:
              "Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6IjY1NmY0NzcxYTYzYTg2YzBmMzM0MTlkZCIsImlhdCI6MTcwMTc5MTYwMiwiZXhwIjoxNzA0MzgzNjAyfQ.A8EKX1v2FCxaKLVO46D4Lp3KrHFksnMaDOzhI3a-9yY",
